...
const command_line::arg_descriptor<uint16_t> arg_daemon_port = { "daemon-port", "Use daemon instance at port <arg> instead of 20635", 0 };
...
bool pool_wallet::confirmTransaction(TransferCommand cmd, bool multiAddress) {
  ...
  if (cmd.fee == 10) {
    feeString = "0.01 LCH (minimum)";
  } else {
    feeString = m_currency.formatAmount(cmd.fee) + " LCH";
  }
  ...
  if (!multiAddress) {
    std::cout << "You are sending " << m_currency.formatAmount(cmd.dsts[0].amount)
              << " LCH, with a fee of " << feeString << std::endl
    ...
    for (auto destination : cmd.dsts) {
      std::cout << "You are sending " << m_currency.formatAmount(destination.amount)
                << " LCH" << std::endl << "FROM: " << walletName << std::endl
...
bool pool_wallet::print_outputs_from_transaction(const std::vector<std::string>& args)
{
  ...
  for (size_t i = 0; i < ourTransaction.outputs.size(); ++i)
  {
    ...
    if (targetPubKey.key == outputPublicKey)
    {
      ...
      std::string LCH = m_currency.formatAmount(amount);

      sum += amount;

      found = true;

      logger(INFO, GREEN) << "The transaction output of " << LCH << " LCH belongs to you!";
    }
  }
  ...
  {
    std::string LCH = m_currency.formatAmount(sum);

    logger(INFO, GREEN) << "Outputs totalling " << LCH << " LCH were sent to your wallet!";
  }
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "LatinCash v" << PROJECT_VERSION << " Poolwallet" << std::endl;
