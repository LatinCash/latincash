        ...
        catch (const std::system_error &e)
        {
            ...
            else if (errMsg == "Network error")
            {
                std::cout << WarningMsg("Couldn't connect to the network to "
                          ...
                          << "Ensure LatinCashd or the remote node you are "
...
Maybe<uint64_t> getFee()
{
    while (true)
    {
        ...
        std::cout << std::endl 
                  ...
                  << std::endl
                  << "Hit enter for the default fee of 0.001 LCH: ";
...
Maybe<uint64_t> getTransferAmount()
{
    while (true)
    {
        ...
        std::cout << std::endl
                  << InformationMsg("How much LCH do you want to send?: ");
...
bool parseFee(std::string feeString)
{
    ...
    if (!parseAmount(feeString, fee))
    {
        std::cout << WarningMsg("Failed to parse fee! Ensure you entered the "
                                "value correctly.")
                  << std::endl
                  << "Please note, you can only use 4 decimal places."
                  << std::endl;
    ...
    {
        std::cout << WarningMsg("Fee must be at least 0.001 LCH!") << std::endl;
        return false;
    }
...
bool parseAddress(std::string address)
{
    ...
    else if (expectedPrefix == 925524 && address.substr(0, 4) != "cash")
    {
        std::cout << WarningMsg("Invalid address! It should start with cash!")
                  << std::endl << std::endl;
        return false;
    }
...
bool parseAmount(std::string amountString)
{
    ...
    if (!parseAmount(amountString, amount))
    {
        std::cout << WarningMsg("Failed to parse amount! Ensure you entered "
                                "the value correctly.")
                  << std::endl
                  << "Please note, the minimum you can send is 0.0001 LCH,"
                  << std::endl
                  << "and you can only use 4 decimal places."
                  << std::endl;
