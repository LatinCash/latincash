/*
Copyright (C) 2018, The LatinCash developers
         ...
         Config &config)
{
    ...
    do
    {
        std::cout << InformationMsg("LatinCash v"
    ...
    {
        std::cout << WarningMsg("It looks like LatinCashd isn't open!")
                  << std::endl << std::endl
                  << WarningMsg("Ensure LatinCashd is open and has finished "
                                "initializing.")
                  << std::endl
                  << WarningMsg("If it's still not working, try restarting "
                                "LatinCashd. The daemon sometimes gets stuck.") 
                  << std::endl
                  << WarningMsg("Alternatively, perhaps LatinCashd can't "
...
std::shared_ptr<WalletInfo> createViewWallet(CryptoNote::WalletGreen &wallet)
{
    ...
    while (true)
    {
        ...
        else if (address.substr(0, 4) != "cash")
        {
            std::cout << WarningMsg("Invalid address! It should start with "
                                    "\"cash\"!") << std::endl;
                                                        ...
                                                        address))
        {
            std::cout << WarningMsg("Failed to parse LCH address! Ensure you "
...
void help(bool viewWallet)
{
    std::cout << "Available commands:" << std::endl
              ...
              << "Display how much LCH you have" << std::endl
    ...
    if (viewWallet)
    {
        std::cout << InformationMsg("Please note you are using a view only "
                                    "wallet, and so cannot transfer LCH.")
                  << std::endl;
    }
    else
    {
                           ...
                           CryptoNote::INode &node)
{
    ...
    /* Couldn't get timestamp, maybe old node or LatinCashd closed */
    if (blockTime != "")
    {
                         ...
                         std::shared_ptr<WalletInfo> &walletInfo)
{
    ...
    if (localHeight != remoteHeight)
    {
        std::cout << "Your LatinCashd isn't fully synced yet!" << std::endl
    ...
    while (walletHeight < localHeight)
    {
        ...
        if (tmpWalletHeight == walletHeight)
        {
            ...
            {
                std::string warning =
                    "Syncing may be stuck. Try restarting LatinCashd.\n"
...
ColouredMsg getPrompt(std::shared_ptr<WalletInfo> &walletInfo)
{
    ...
    return InformationMsg("[LCH " + shortName + "]: ");
}

void connectingMsg()
{
    std::cout << std::endl << "Making initial contact with LatinCashd."
              << std::endl
...
void viewWalletMsg()
{
    std::cout << InformationMsg("Please remember that when using a view wallet "
              ...
              << std::endl << "This means if you received 100 LCH and then "
              << "sent 50 LCH, your balance would appear to still be 100 "
              << "LCH." << std::endl
